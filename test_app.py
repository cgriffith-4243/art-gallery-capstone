...
class AppTestCases(unittest.TestCase):
    ...
    def test_create_medium_valid_data(self):
        ...
        response = self.client() \
            .post('/mediums', json=test_medium, headers=HEADERS)
    ...
    def test_create_artwork_valid_data(self):
        test_artwork = {
            ...
            'medium': 'oil painting',
            'year': 2016,
            'image_link': ('https://i.picsum.photos/id/1056/3988/2720.jpg?hmac'
                           '=qX6hO_75zxeYI7C-1TOspJ0_bRDbYInBwYeoy_z_h08'),
            'medium_id': 2
        }
        total_before = len(Artwork.query.all())
        response = self.client() \
            .post('/artworks', json=test_artwork, headers=HEADERS)
        data = json.loads(response.data)
    ...
    def test_update_medium_valid_id(self):
        ...
        response = self.client() \
            .patch('/mediums/{}'.format(test_id), json=medium, headers=HEADERS)
        data = json.loads(response.data)
        # check response
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['success'], True)
    ...
    def test_update_medium_unauth(self):
        ...
        response = self.client() \
            .patch('/mediums/{}'.format(test_id), json=medium)
        data = json.loads(response.data)
        # check response
        self.assertEqual(response.status_code, 401)
        self.assertEqual(data['success'], False)
    ...
    def test_update_artwork_valid_id(self):
        ...
        artwork = {
            ...
            'year': 2012,
            'image_link': ('https://i.picsum.photos/id/103/2592/1936.jpg?hmac'
                           '=aC1FT3vX9bCVMIT-KXjHLhP6vImAcsyGCH49vVkAjPQ'),
            'medium_id': 2
        }
        response = self.client().patch('/artworks/{}'.format(test_id),
                                       json=artwork, headers=HEADERS)
        data = json.loads(response.data)
        # check response
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['success'], True)
    ...
    def test_update_artwork_unauth(self):
        ...
        response = self.client() \
            .patch('/artworks/{}'.format(test_id), json=artwork)
        data = json.loads(response.data)
        # check response
        self.assertEqual(response.status_code, 401)
        self.assertEqual(data['success'], False)
    ...
    def test_delete_medium_valid_id(self):
        ...
        total_before = len(Medium.query.all())
        response = self.client() \
            .delete('/mediums/{}'.format(test_id), headers=HEADERS)
        data = json.loads(response.data)
        total_after = len(Medium.query.all())
        # check response
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_artwork_valid_id(self):
        ...
        total_before = len(Artwork.query.all())
        response = self.client() \
            .delete('/artworks/{}'.format(test_id), headers=HEADERS)
        data = json.loads(response.data)
        total_after = len(Artwork.query.all())
        # check response
        self.assertEqual(response.status_code, 200)
